...
Default pipeline: inbound PCM is routed through VAD and Whisper, response text is generated by a local pipeline (echo stub or a local Ollama model), and XTTS-v2 streams outbound PCM back into the call. Both directions work — the decisive detail is the call-setup ordering, see the inbound-audio note below.
